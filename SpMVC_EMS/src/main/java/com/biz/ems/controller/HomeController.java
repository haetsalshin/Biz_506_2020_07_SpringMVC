...
public class HomeController {
	 ...
	 * id값을 변수로 넘기면서 update, GET 로 호출이 된다.
	 * id값으로 findById() 조회를 해서 EmsVO를 DB 로 부터 Select하고
	...
	public String update(EmsVO emsVO,
		...
		if(!file1.getOriginalFilename().isEmpty()) {
			...
			// 3-6. upload된 파일이 없으면
		}else if(file1.getOriginalFilename().isEmpty()) { // 파일이 업로드가 되지 않았을 때
		...
		}else if(file2.getOriginalFilename().isEmpty()) { // 파일이 업로드가 되지 않았을 때
			emsVO.setS_file2(emsOldVO.getS_file2()); // 원래 있던 파일 이름을 복제해서 사용하라
		}

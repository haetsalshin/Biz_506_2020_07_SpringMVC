...
public class MemberController {
	...
	/*
	 * Authentication : 로그인한 사용자 정보를 authProvider에 담아준다.
	 */
	...
	public UserDetailsVO userInfo(Principal pricipal,
			Authentication authentication,
			@ModelAttribute("memberVO") UserDetailsVO userVO, Model model) {
		
		
		//spring security 프로젝트에서 로그인한 사용자 정보를 추출하는 여러가지 방법
		// 1. UserDetailsServiceImpleVI1에서 공급받는 방법
		// 	서버의 Session memory에 직접 접근하여 사용자 정보를 추출하는 방법으로
		//	보안에 상당히 취약해서 사용을 지양하는 방법
		UsernamePasswordAuthenticationToken upa = (UsernamePasswordAuthenticationToken) pricipal;
		...
		// 3. Authenticatin 클래스를 매개변수로 설정하는 방법
		// @AuthenticationPricipal이 작동이 안되는 관계로
		// 매개변수에 @Authentication 클래스를 객체로 선언하고
		// authentication.getPrincipal() method를 호출하여 userVO를 추출하는 방법
		// 이 방법이 지금 가장 권장하는 방법이다.

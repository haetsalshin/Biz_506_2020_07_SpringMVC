 ...
 * }
 * 
 * 이를 선언하려면 반드시 private final로 선언되어 있어야 한다
 * 
 */
...
public class MemberServiceImplV1 implements MemberService {
	...
	public int insert(UserDetailsVO userVO) {
		...
		/*
		 * 회원 테이블에 값이 없을 때 (0일 때) 회원가입이 이루어지면
		 * 그 회원은 admin권한을 갖고
		...
		// 1. 회원가입한 모든 사용자에게 기본적으로 ROLL_USER 권한을 부여하고
		...
		// List에 담겨 있는 값을 DB insert할 때
		// 가장 쉽게 할 수 있는 방법이 list를 for반복문으로 감싸고
		// vo에 한개씩을 insert에 보내는 방법
		// 촌스러운 방법이다.
...
//		}
		...
		// 평문으로 입력된 비밀번호를 암호화 된 비밀번호로 대치
		userVO.setPassword(encPassword);
		
		// userDao의 insert가 수행되기 전에 authDao.insert가 수행되고 insert가 될 것이고
	...
	public String userNameAndPassword(String username, String password) {
		...
		// 결과값에 따라 3항 연산자를 사용하여 OK 또는 FAIL문자열을
		// Controller로 return

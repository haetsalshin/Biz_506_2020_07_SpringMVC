...
public class MemberServiceImplV1 implements MemberService {
	 ...
	 *  권한(ROLL)을 ADMIN으로 부여한다.
	 *  2. 2번째부터 가입된 회원은 권한(ROLL)을 user로 부여한다.
	 *  3. 회원이 탈퇴를 요청하면 권한(ROLL)을 "GUEST"로 변경한다.
	 *  4. 같은 ID로 재 가입을 요청하면 권한을 GUEST에서 USER
	 *  	로 변경한다.
	 *  5. ADMIN 권한은 현재 App의 모든 기능을 사용할 수 있다.
	 *  	- 다른 사용자의 정보를 변경 할 수 있다.
	 *  	- 다른 사람의 권한(ROLL) 을 변경할 수 있다.
	 *  6. USER권한은 ADMIN권한으로 접근할 수 있는 부분은 사용이 불가하다
	 *  	- 본인의 사용자 정보만 변경할 수 있다.
	 *  	- 본인의 사용자 권한은 변경할 수 없다.
	 *  7. GUEST권한은 로그인과, 재가입 요청만 할 수 있다.
	 *   
	 * * 회원가입 절차
	...
	public int insert(MemberVO membVO) {
		...
		// tbl_member table에 저장된 record 개수 가져오기
		 ...
		 * 데이터 없을 때, 0이거나 -1인 경우가 된다면
		 * if(count == 0 || count == -1)또는
		 * if(count <= 0 )과 같은 비교연산식이 만들어 져야 한다.
		 * 
		 * 비교 연산식에서 가장 권장하는(좋은 코드)는
		 * 가급적 비교 대상을 부등호로 한번만 비교하는 것이다
		 * else가 있는 상황이므로
		 * 더 값을 근접하게 찾는 연산식으로 변경하는 것이 좋다.
		 * 
		 * if(count >0)이 이상적인 코드이다.
		 */
		
		// 회원정보가 없으면 ADMIN, 있으면 USER
		if(count > 0) {
			membVO.setM_roll("USER");
		}else {

...
public class ProductServiceImplV1 implements ProductService {
	 ...
	 * Master Data는  
	 * Work Data (매입매출, 거래처 외상정보, 회원로그인 로그와 같은)의 
	 * 기준이 되는 값들을 가지는 Data Table들이다.
	 * 
	 * 어플을 사용해서 어떤 업무를 진행하는 과정에서
	 * workData에 저장되는 데이터는 필수적으로 Master Data와 Join관계가 맺어진다.
	 * 
	 * 만약 WorkData와 Master Data를 Join하여 어떤 통계정보를 찾고할 때
	 * Master에 해당하는 데이터(코드와 일치하는)가 없으면
	 * WorkData의 정보가 어떤 내용인지 확인 어려워 지는 경우가 많다.
	 * 
	 * 통상적으로 Master Data는 원칙상 PK 칼럼 값은 변경하지 않고
	 * 또한 한번 insert 된 Master Data는 삭제하지 않는다.
	 * 
	 * 필요가 없어진(앞으로 사용하지 않을) 데이터는 
	 * table에서 delete를 하지 않고 특정한 칼럼을 한개 지정하여
	 * 그 칼럼에 flag를 setting하고 삭제 되었음을 표시한다.
	 * 
	 * 
	 * 
	 ...
	 * 3. update를 수행한다.
	 * 4. select를 수행할 때 p_not_use칼럼이 null인 데이터만 조회하여
	...
	public int delete(String id) {
		...
		int ret = 0;
		if(proVO != null) {
			proVO.setP_not_use((byte)1); // 숫자 1이 아닌 8bit형 으로 저장된다.
			proDao.update(proVO);
			ret = proDao.update(proVO);
		}
		// Controller에서 delete() method를 실행 한 후 결과값을 보고

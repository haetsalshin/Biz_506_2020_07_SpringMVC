...
/*
 *	RestController
 *	표준 Controller는 return문자열을 가지고 view파일을 열고
 *	rendering을 수행한 후 web client에 HTML 코드로 response를 수행한다.
 *	그에 반해 RestController는 view가 없이 데이터를 직접 web client에 전송할 떄 사용하는 controller
 *
 * 	표준 Controller에서는 특정한 method를 지정하여 view가 아닌 
 * 	데이터를 직접 web client로 전송할 때
 * 	method에 @ResponseBody를 설정해준다.
 * 
 * 	RestController는 모든 method에 @ResponseBody가 설정된 것과 똑같은 효과를 낸다.
 * 
